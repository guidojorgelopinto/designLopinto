...
export class HomeComponent implements OnInit {
  constructor(private cookieService: CookieService, private headerService: HeaderService, private elementRef: ElementRef) { }

    showAcceptCookiesBanner = false;

  ngOnInit(): void {
    this.headerService.setSticky(false); // Desactiva el sticky en el header

    const cookiesAccepted = this.cookieService.get('cookiesAccepted');
    this.showAcceptCookiesBanner = !cookiesAccepted;
  }
